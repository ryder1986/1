...
def get_objects1(source_image, filtered_objects, w,h):
    # copy image so we can draw on it. Could just draw directly on source image if not concerned about that.
    display_image = source_image.copy()
    source_image_width = source_image.shape[1]
    source_image_height = source_image.shape[0]
...
#frames_per_second = frame_count / (end_time - start_time)
    ...
    for obj_index in range(len(filtered_objects)):
        ...
        box_left=box_left*w/TY_NETWORK_IMAGE_WIDTH
        box_top=box_top*h/TY_NETWORK_IMAGE_HEIGHT
        box_w=box_w*w/TY_NETWORK_IMAGE_WIDTH
        box_h=box_h*h/TY_NETWORK_IMAGE_HEIGHT
        objs=objs+[box_left,box_top,box_w,box_h]
    return objs
